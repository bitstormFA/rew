...
proc reduceMean*(t: Tensor; dims: openArray[int]): Tensor =
  ...
  let divisor = scalarF32(float32(n))
...
proc argmax*(t: Tensor; dim: int; keepdims = false): Tensor =
  ...
  let largeVal = scalarI32(int32.high)
...
proc logSumExp*(t: Tensor; dims: openArray[int]): Tensor =
  ...
  let maxVals = reduceMax(t, dims)
  var bdims: seq[int] = @[]
  for i in 0 ..< t.shape.len:
    if i notin dims: bdims.add i
  let maxB = broadcastTo(maxVals, t.shape, bdims)
  let shifted = sub(t, maxB)
  let expShifted = exp(shifted)
  let sumExp = reduceSum(expShifted, dims)
...
proc variance*(t: Tensor; dims: openArray[int]; unbiased = true): Tensor =
  ...
  let meanVals = reduceMean(t, dims)
  var bdims: seq[int] = @[]
  for i in 0 ..< t.shape.len:
    if i notin dims: bdims.add i
  ...
  let sumSq = reduceSum(sq, dims)
  var n = 1
  for d in dims: n *= t.shape[d]
  if unbiased and n > 1:
    let denom = scalarF32(float32(n - 1))
    var zeroDims: seq[int] = @[]
    let denomB = broadcastTo(denom, sumSq.shape, zeroDims)
    divide(sumSq, denomB)
  else:
    let denom = scalarF32(float32(n))
    var zeroDims: seq[int] = @[]
    let denomB = broadcastTo(denom, sumSq.shape, zeroDims)
    divide(sumSq, denomB)
...
proc norm*(t: Tensor; p: float32 = 2'f32; dims: openArray[int] = []): Tensor =
  ...
  ## norm.
  var dimsToReduce = @dims
  if dimsToReduce.len == 0:
    for i in 0 ..< t.shape.len: dimsToReduce.add i
  if p == 2'f32:
...
proc trace*(a: Tensor; offset: int = 0; axis1: int = 0;
  ...
  var i1Shifted = i1
  if offset != 0:
    let off = scalarI32(int32(offset))
...
proc det*(a: Tensor; lower = true): Tensor =
  ...
  let two = broadcastTo(scalarF32(2'f32), sumLogDiag.shape, [])
...
proc average*(t: Tensor; dims: openArray[int] = []): Tensor =
  ...
  ## this is equivalent to `reduceMean(t, dims)`.
  reduceMean(t, dims)
...
proc cov*(x: Tensor; rowvar: bool = true): Tensor =
  ...
  let denom = broadcastTo(scalarF32(float32(n - 1)),
    covMat.shape, @[])
  divide(covMat, denom)
